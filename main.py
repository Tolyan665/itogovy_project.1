...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        # Кнопка добавления
        self.img_add = tk.PhotoImage(file='image/add.png')
        btn_add = tk.Button(toolbar, text='Add', bg='#d7d7d7',
                            bd=0, image=self.img_add, command=self.open_child)
        btn_add.pack(side=tk.LEFT)
        # Кнопка изменения
        self.img_upd = tk.PhotoImage(file='image/изменение.png')
        btn_upd = tk.Button(toolbar, text='Upd', bg='#d7d7d7', bd=0, image=self.img_upd, command=self.open_update_child)
        btn_upd.pack(side=tk.LEFT)
        # Кнопка удаления
        self.img_del = tk.PhotoImage(file='image/удаление.png')
        btn_del = tk.Button(toolbar, bg='#d7d7d7',
                            bd=0, image=self.img_del,
                            command=self.delete_records)
        btn_del.pack(side=tk.LEFT)
        # Кнопка поиска
        self.img_search = tk.PhotoImage(file='image/поиск.png')
        btn_search = tk.Button(toolbar, bg='#d7d7d7',
                            bd=0, image=self.img_search,
                            command=self.open_search)
        btn_search.pack(side=tk.LEFT)
        # Кнопка обновления
        self.img_refresh = tk.PhotoImage(file='image/перезагрузка.png')
        btn_refresh = tk.Button(toolbar, bg='#d7d7d7', bd=0, image=self.img_refresh, command=self.view_records)
